...
IP_ADDR = '127.0.0.1'
PORT = 5000     # Start Pseudo-Utlra96 server on a random port
PORT_NUMS = [8080,8081,8082,8083]
...
class Server(threading.Thread):
    ...
    def setup_connection(self):
        ...
        while True:
            ...
            db_thread1 = threading.Thread(target=self.run, args=(conn1,db_addr1))
            ...
            db_thread2 = threading.Thread(target=self.run, args=(conn2,db_addr2))
            ...
            db_thread3 = threading.Thread(target=self.run, args=(conn3,db_addr3))
            ...
            db_thread4 = threading.Thread(target=self.run, args=(conn4,db_addr4))
    ...
    def run(self,conn,addr):
        while not self.shutdown.is_set():
            ...
            test_packet_msg = '# 1 | James Bond | 3'

            if is_send:
                print('Sending data to db via port: ' + str(addr[1]))
                conn.sendall(test_packet_msg.encode(FORMAT))
